...
import { adminUser, operatorUser } from 'app/mock-api/common/user/data';
...
export class AuthMockApi
{
    ...
    private _adminUser: User = adminUser;
    private _operatorUser: User = operatorUser;
    ...
    registerHandlers(): void
    {
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
              ...
              if (request.body.username === 'admin' && request.body.password === 'CMDCloudOrder') {
                user = this._adminUser;
              }
          
              if (request.body.username === 'operatore') {
                user = this._operatorUser;
              }
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                // Verify the token
                if ( AuthUtils._decodeToken(accessToken).username == this._adminUser.username )
                {
                  user = this._adminUser;
                }

                if ( AuthUtils._decodeToken(accessToken).username == this._operatorUser.username )
                {
                  user = this._operatorUser;
                }
     ...
     * @private
     */
    private _generateJWTToken(user: User): string
    {
        // Define token header
        const header = {
            alg: 'HS256',
